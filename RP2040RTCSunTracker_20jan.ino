/*(c)Joyfab. Two axis Azimut/Elevation RTC Suntracker.
  (RP2040-Zero) Joy nov 2024.*/
#include <Arduino.h>
#include <Wire.h>
#include "PioSPI.h"
...
void wind() {
  ...
  stepper2.runToPosition();          // Night Wind position
  pixels.clear();
  pixels.setPixelColor(0, pixels.Color(1, 0, 0));
  pixels.show();
}
void track() { // ex: 5°37'43.66"E,44°18'58.91"N => 5.62879,44.31636 decimal. see https://www.coordonnees-gps.fr/conversion-coordonnees-gps
  ...
  float ElReach = ((dElevation * 64000 / 9) + (ElOffset * 1000) - 100000) - 32000; // dElevation * (200steps/rev * 256µsteps/step * 100ratioreduct) / 360°) + Offsett - Limit minimum
  float AzReach = ((dAzimuth * 64000 / 9) + (AzOffset * 1000) - 100000) - 512000 ; // = 14222,22 µsteps / degree.
  if (ElReach >= 600000) {  // Limit maximum Elevation (85°)
    ElReach = 600000;
  }
  if (ElReach <= 0) {   // Limit minimum Elevation (5°)
    ElReach = 0;
  }
